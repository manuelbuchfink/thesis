...
import tensorboardX
from ct_2d_limited_angle_projector import FanBeam2DProjectorLimitedAngle
...
ct_projector_full_view_512 = FanBeam2DProjectorLimitedAngle(config['img_size'], config['proj_size'], config['num_proj_full_view_512'], config['batch_size'])
ct_projector_sparse_view_128_limit = FanBeam2DProjectorLimitedAngle(config['img_size'], config['proj_size'], config['num_proj_sparse_view_128'], config['batch_size'])
ct_projector_sparse_view_64_limit = FanBeam2DProjectorLimitedAngle(config['img_size'], config['proj_size'], config['num_proj_sparse_view_64'], config['batch_size'])
...
for it, (grid, image) in enumerate(data_loader):
    for i in range(config['batch_size']):
        ...
        projections[i] = (ct_projector_sparse_view_128_limit.forward_project(images[i].transpose(1, 3).squeeze(1)))  # ([1, y, x])        -> [1, num_proj, x]
        fbp_recon_128 = ct_projector_sparse_view_128_limit.backward_project(projections[i])                    # ([1, num_proj, x]) -> [1, y, x]

        # projs_64 = ct_projector_sparse_view_64_limit.forward_project(images[i].transpose(1, 3).squeeze(1))
        # fbp_recon_64 = ct_projector_sparse_view_64_limit.backward_project(projs_64)  
    ...
    for it, (grid, image) in enumerate(data_loader): 
        for i in range(config['batch_size']):  
            ...
            for iterations in range(max_iter):
                ...
                train_projs = ct_projector_sparse_view_128_limit.forward_project(train_output.transpose(1, 3).squeeze(1)).to("cuda")      # evaluate by forward projecting
