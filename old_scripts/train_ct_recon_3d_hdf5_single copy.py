...
for it, (grid, image) in enumerate(data_loader):
    ...
    fbp_recon = fbp_recon.squeeze()[::n,::n,::n].unsqueeze(0).unsqueeze(4).cuda()    # down-sample by factor 4
    grid = grid.squeeze()[::n,::n,::n].unsqueeze(0).cuda()                   # [1, h, w, d, 3], value range = [0, 1]

    ct_projector_sparse_view_small = ConeBeam3DProjector(fbp_recon.squeeze().shape, proj_size=config['proj_size'], num_proj=config['num_proj_sparse_view'])
    ...
    for iterations in range(max_iter):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            ...
            train_projections = ct_projector_sparse_view_small.forward_project(train_output.transpose(1, 4).squeeze(1)).to("cuda")      # evaluate by forward projecting
