...
for i in range(100):
    ...
    optim = torch.optim.Adam(model.parameters(), lr=new_lr, betas=(config['beta1'], config['beta2']), weight_decay=config['weight_decay'])
    ...
    for it, (grid, image) in enumerate(data_loader):
        ...
        limit_projs = torch.chunk(train_proj128, 4, axis=1)
        for i in range(len(limit_projs)):
            save_image_2d(limit_projs[i], os.path.join(image_directory, f"train128_{i}.png"))
