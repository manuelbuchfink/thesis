...
for it, (grid, image) in enumerate(data_loader):
    ...
    print(f"rcon shape {fbp_recon.shape}")
    ct_projector_sparse_view = ConeBeam3DProjector(fbp_recon.squeeze().shape, config['cb_para'])
    ...
    for iterations in range(max_iter):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            ...
            #train_projections = ct_projector_sparse_view.forward_project(train_output.transpose(1, 4).squeeze(1))
