...
for it, (grid, image) in enumerate(data_loader):
    ...
    #prior_volume = F.interpolate(torch.tensor(prior_volume, dtype=torch.float16), size=(512, 512, 512), mode='trilinear') #up-sample by repeating values based on nearest neighbors
    prior_volume = prior_volume.squeeze().unsqueeze(0).unsqueeze(4)
