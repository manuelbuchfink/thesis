...
class Initialization_ConeBeam:
    def __init__(self, image_size, num_proj, start_angle, proj_size):
        ...
        ## Detector
        self.param['sh'] = self.param['sx'] * (2.2128) # Size of a detector pixel.
        self.param['sw'] = np.sqrt(self.param['sx']**2 + self.param['sy']**2) * (2.2128)
        self.param['nh'] = proj_size[0]
        self.param['nw'] = proj_size[1]
        '''
        dde = source_to_detector - source_to_isocenter = 1085,6 - 595 = 490,6
        ODD = SDD                - SOD                 = 1085,6 - 595 = 490,6
        magnification = SDD / SOD                      = 1085,6 / 595 = 2,2128
        '''
...
def build_conebeam_geometry(param):
    ...
    angle_partition = odl.uniform_partition(min_pt=param.param['start_angle'] + 1/4*2*np.pi, # + angle shift ctutil definition
                                            max_pt=param.param['end_angle'] + 1/4*2*np.pi,
