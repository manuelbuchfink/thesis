...
from utils import get_config, prepare_sub_folder, get_data_loader, ct_parallel_project_2d_batch, save_image_2d
...
ct_projector = FanBeam2DProjector(config['img_size'], config['num_proj'])
...
for it, (grid, image) in enumerate(data_loader):
    ...
    thetas = torch.linspace(0, 2 * np.pi, config['num_proj']+1)[:-1].cuda() #projection angles

    projs = ct_parallel_project_2d_batch(image, thetas)  # [bs, n, w, c]
    #print(image.transpose(1,3))
    projs2 = ct_projector.forward_project(image.transpose(1, 3).squeeze(1))
    fbp_recon = ct_projector.backward_project(projs2)  # [bs, n, h, w] -> [bs, x, y, z]
    ...
    save_image_2d(test_data[1], os.path.join(image_directory, "test2.png"))
    save_image_2d(train_data[1], os.path.join(image_directory, "train2.png"))
    ...
    fbp_recon = fbp_recon.unsqueeze(1).transpose(1, 3)  # [bs, z, x, y, 1]
    fbp_recon_psnr = - 10 * torch.log10(loss_fn(fbp_recon, test_data[1]))
    save_image_2d(fbp_recon, os.path.join(image_directory, "fbprecon2.png"))
